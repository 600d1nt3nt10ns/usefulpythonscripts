...
# Check dynamically changing ports for wildcard marking
def is_dynamic_port(port):
    dynamic_port_range = range(30000, 65536)
    return int(port) in dynamic_port_range

# Substitute dynamically changing ports with wildcards and filter similar records
...
def generate_iptables_rules(wildcarded_flow):
    ...
    for src_ip, src_port, dst_ip, dst_port in input_for_rules:
        ...
        rules_dict[dst_ip].add(f"iptables -A INPUT -p tcp -m tcp {src_rule} {dst_rule} -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT")
    return rules_dict
...
#print(wildcarded_flow)

# Generate iptables rules
iptables_rules = generate_iptables_rules(wildcarded_flow)

# Print generated iptables rules to a file
with open("generated_iptable_rules.txt", "w") as generated_iptables_rules:
